...
//Append the tooltips to call later
svg.append('div')
    .attr('class', 'tooltip')
    .style('opacity', 0)    
...
d3.csv(csv).then(function(data) {
    ...
    //Cast each value of dataset as number 
    ...
    //Static Chart Axes(poverty vs. smoking)
    ...
    svg.selectAll('text')
        ...
        .attr('x', function(d) {
            return xScale(d[data.poverty])
        })
        .attr('x', function(d) {
            return yScale(d[data.smokes])
        })
        // .attr('font-size', '12px')
    ...
    //X Axis
    svg.append('text')
        .attr('transform', `translate(${chartWidth - 40}, ${chartHeight})`)
        .attr('class', 'x-axis-label active')
        .attr('axis-name', 'poverty')
        .text('In Poverty (%)')
    svg.append('text')
        .attr('transform', `translate(${chartWidth - 40}, ${chartHeight})`)
        .attr('class', 'x-axis-label inactive')
    ...
    svg.append('text')
        ...
        .attr('class', 'y-axis label active')
        .attr('axis-name', 'obesity')
        .text('Obesity (%)')
    svg.append('text')    
        .attr('transform', `translate(-40, ${height /2})rotate(270)`)
        .attr('class', 'y-axis label inactive')
        .attr('axis-name', 'healthcare')
        .text('% of People with Access to Healthcare')
    svg.append('text')
        .attr('transform', `translate(-40, ${height /2})rotate(270)`)
        .attr('class', 'y-axis label inactive')
        .attr('axis-name', 'smokers')
        .text('Smokers (%)')

    //Enable dynamic axes-- Add active an inactive labels for styling
    function xAxisChange(axisSelection) {
    ...
    function yAxisChange(axisSelection) {
    ...
    //Change the data along with the axes
    d3.selectAll('.x-axis-label').on('click', function() {

        var clickedSelection = d3.select(this);
        var isSelectionInactive = clickedSelection.classed('inactive');
